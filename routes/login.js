    ...
    function(req, username, password, done) {
        if (!checkAlpha(username) || !checkAlpha(password)) {
            return done(null, false, {
                message: "Username and Password can contain only alphabets,numbers, _ or -"
            });
        }
...
passport.use('register', new LocalStrategy({
    ...
    function(req, username, password, done) {
        if (!checkAlpha(username)) {
            return done(null, false, {
                message: "Username can contain only alphabets,numbers, _ or -"
            });
        }
        pool.getConnection(function(err, connection) {
            connection.query('SELECT * from users.user where username=\"' + username + '\"', function(err, rows, fields) {
...
router.get('/', function(req, res) {
    if (req.flash('login') == 'LOGIN') {
        ...
        res.render('login', {
            title: 'Login',
            errormess: 'Login to access site'
        });
    } else
        res.render('login', {
            title: 'Login'
